...
class LISA():
    ...
    def params_dump(self):
        ...
        for j in range(self.data_dim - 1):
            ...
            one_dim_front_split_points = np.zeros_like(one_dim_split_upper_bounds,
                                                       dtype=one_dim_split_upper_bounds.dtype)
        ...
        self.last_dim_front_split_points = np.zeros_like(self.last_dim_split_upper_bounds,
                                                         dtype=self.last_dim_split_upper_bounds.dtype)
    ...
    def monotone_mappings(self, data):
        ...
        for i in range(1, data.shape[1] - 1):
            for j in range(idxes.shape[0]):
                idxes[j] = idxes[j] * self.n_parts_each_dim + np.searchsorted(
                    self.all_split_points_without_head_and_tail[i][idxes[j]], data[j, i], side='right')
        ...
        mappings = measures * self.eta + (last_dim_data / self.max_value_each_dim * (self.n_parts_each_dim - 1)) + (
    ...
    def get_intersect_shard(self, query_range, dim, cell_idx_base, lower_measure, upper_measure, lower_mappings,
                            upper_mappings):
    ...
    def get_query_ranges_mappings(self, query_ranges):
        ...
        for i in range(query_ranges.shape[0]):
            ...
            self.union_continuous_cells(single_qr_lower_mappings, single_qr_upper_mappings, lower_mappings_list,
                                        upper_mappings_list)
        ...
        for i in range(points.shape[0]):
            ...
            if flag == False:
                ...
                print('i =', i, ', point =', points[
                    i], ', point_mapping =', point_mapping, ', lower_bound =', lower_bound, ', upper_bound =',
                      upper_bound)
    ...
    def delete_within_shard(self, point, point_mapping, shard_id):
        ...
        if len(shard_page_nos) == 2:
            ...
            if page_idx == 0:
                ...
                if self.m_counts[shard_page_nos[max_idx - 1]] + self.m_counts[
                    shard_page_nos[max_idx]] <= self.page_size:
                    left_idx = max_idx - 1
            else:
                if (self.m_counts[shard_page_nos[page_idx]] + self.m_counts[
                    shard_page_nos[page_idx - 1]] <= self.page_size):
                    left_idx = page_idx - 1
                elif self.m_counts[shard_page_nos[page_idx]] + self.m_counts[
                    shard_page_nos[page_idx + 1]] <= self.page_size:
                    left_idx = page_idx
            if left_idx >= 0:
